        ...
        if not User.objects.filter(username=username).exists():
            if not User.objects.filter(email=email).exists():
                if len(password) < 6:
                    ...
                    user.is_active = True  # Set active for development
                    user.save()
                    ...
                    email_body = 'Hi '+user.username + \
                        ' Please use this link to verify your acount\n' + activate_url
                    email = EmailMessage(
                        email_subject,
                        email_body,
                ...
                messages.error(
                    request, 'Sorry email in use, choose another email')
                return render(request, 'authentication/register.html')
        else:
...
class RequestPasswordResetEmail(View):
    ...
    def post(self, request):
        ...
        if user.exists():
            ...
            link = reverse('authentication:reset-user-password', kwargs={
            ...
            messages.info(
                request, 'User with this email id does not found, please register')
            return render(request, 'authentication/reset-password.html')
