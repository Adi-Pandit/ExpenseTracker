...
def search_budget(request):
    if request.method == 'POST':
        ...
        budget = Budget.objects.filter(amount__istartswith=search_str, owner=request.user) | Budget.objects.filter(date__istartswith=search_str, owner=request.user) | Budget.objects.filter(
            description__icontains=search_str, owner=request.user) | Budget.objects.filter(source__istartswith=search_str, owner=request.user)
        data = budget.values()
...
def index(request):
    ...
    budgetset = "set"
    try:
        budgets = Budget.objects.get(
            owner=request.user, month=month, year=year)
...
def add_budget(request):
    ...
    if request.method == 'POST':
        ...
        for k, v in final.items():
            Budget_amount.objects.create(
                category=k, amount=v, budget_id=budget_last.id)
...
def budget_source_summary(request):
    ...
    print(category)
    finalrep = {'Budget amount': 1000,
                'Spend amount': 200, 'Remaining amount': 800}
...
def bstats_view(request):
    ...
    if day in range(1, last+1):
        try:
            Budget_id = Budget.objects.get(
                owner=request.user, month=monthName, year=year)
        except:
            context = {
                'msg': 'unset'
            }
        ...
        for category in categoryList:
            amount = 0
            budget_amount = Budget_amount.objects.get(
                category=category, budget_id=Budget_id.id)
            expenses = Expense.objects.filter(
                owner=request.user, category=category, date__gte=start_date, date__lte=last_date)
...
def bexport_pdf(request):
    ...
    total = budgets.aggregate(Sum('amount'))['amount__sum'] or 0

    # Create PDF response
    response = HttpResponse(content_type='application/pdf')
    response['Content-Disposition'] = 'attachment; filename="budgets.pdf"'
    ...
    # Title
    title = Paragraph("Budget Report", title_style)
    elements.append(title)
    elements.append(Paragraph("<br/>", styles['Normal']))

    # Table data
    data = [['No', 'Amount', 'Source', 'Description', 'Date']]
